...
The conservative development-v0.3 gate accepts only direct evidence at the calibrated 0.65 threshold and treats partial evidence as abstention. It records zero direct false acceptances on both static and online structural development oracles, including leave-one-repository-out validation. Direct precision is 100%, while acceptance coverage is intentionally low at 3.33% static and 5.00% online. These are development-oracle results, not a universal semantic guarantee; a blinded 49-case relevance census is awaiting two independent human reviewers.
