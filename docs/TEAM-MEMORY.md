...
Qarinah is **evidence-linked project memory for coding agents - compact enough to save context, inspectable enough to trust**. Its source of truth is an explicitly authorized, append-only project record. Every compiled memory item carries an event ID and content hash so a person or agent can follow it back to evidence.
