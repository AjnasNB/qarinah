...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Static | 49 | 8/240 | 0/49 | 100% (63.06%-100%) | 0% (0%-7.25%) | 4.19% | 3.33% |
| Online/prequential | 31 | 12/240 | 0/31 | 100% (73.54%-100%) | 0% (0%-11.22%) | 5.74% | 5.00% |
...
The final protocol is frozen at tag `research-protocol-v1`. Its deterministic Verified-minus-Lite manifest contains 387 eligible retrieval tasks across 12 repositories and a frozen 40-task agent sample. It excludes 93 tasks already present in the Lite development corpus. Amendment A001 retains the separate 20 tasks without chronologically prior same-repository development memory as abstention-only negative controls; they remain excluded from positive retrieval recall and coding-handoff utility. The source is the 500-row SWE-bench Verified test artifact at revision `c104f840cc67f8b6eec6f759ebc8b2693d585d4a`, SHA-256 `a45b1fe4e2f0c8390b2b2938ac83e92ed5979000856808f3679c07812e9e6dcd`. Both manifests record `resultsObserved: false`; no final Qarinah retrieval or model result has been run.

The pre-outcome contamination audit covers all 407 final/control tasks against all 300 Lite development tasks. It finds zero exact instance, issue URL, gold-patch hash, test-patch hash, normalized problem-statement hash, and >=0.85 five-token-shingle near-duplicate overlaps. Five pairs share a repository-qualified base commit but have different issues and patches; this is preserved rather than mislabeled as contamination. There are 13,640 same-repository source records at or after target time across all task-record comparisons, and the frozen strict-before admission rule admits zero of them.

The paired power analysis confirms that 40 tasks are underpowered for modest binary resolution gains. At 20% pair discordance, an exact two-sided McNemar test reaches 80% power only around a 20 percentage-point net improvement; at 30%-40% discordance the explored thresholds are approximately 24.5-28.5 points. SWE-bench resolution is therefore a secondary outcome unless the effect is large. The primary study framing remains portable handoff, supplied context, repeated work, citations, and invalid-evidence exposure. No final outcomes were inspected for this analysis.

The recorded local readiness audit passes the complete release gate on Windows x64 with Node 24.15.0, Codex CLI 0.144.6, Claude Code 2.1.118, and Docker 29.3.1. The Codex and Claude packaged runtimes are byte-identical and pass MCP smoke. The repository declares a nine-cell Ubuntu/macOS/Windows by Node 22/24/26 CI matrix; remote results must be reported from the published pull request rather than inferred locally. Provider model resolution, dense and summary baselines, independent human review, a successful provider pilot, and the official Docker gold smoke remain hard blockers.
...
- Frozen abstention controls: [`bench/final/final-abstention-controls-v1.json`](../bench/final/final-abstention-controls-v1.json)
- Pre-outcome contamination audit: [`bench/final/contamination-audit-v1.json`](../bench/final/contamination-audit-v1.json)
- Paired power analysis: [`bench/final/paired-power-analysis-v1.json`](../bench/final/paired-power-analysis-v1.json)
- Protocol amendment receipt: [`bench/final/protocol-amendment-001.json`](../bench/final/protocol-amendment-001.json)
