...
Qarinah is a local project-memory and context-compilation tool for coding agents. It keeps permitted project events and explicit decisions in an evidence-linked record, then retrieves a small, cited context pack for the current task. The goal is to avoid replaying an entire retained project history when Codex, Claude Code, a CLI workflow, or another reviewed host needs only a few relevant records.
