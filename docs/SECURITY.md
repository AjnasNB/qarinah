...
- Claude binds plugin execution to a required user-reviewed `node_path` file setting. Codex currently has no equivalent per-plugin setting: its hooks and MCP server first change to the immutable installed-plugin root to defeat workspace-current-directory shadowing, but the reviewed local plugin still inherits the host's remaining executable-search-path trust boundary.
