# Qarinah launch media matrix

| Surface | Asset | Native size | Use |
| --- | --- | ---: | --- |
| GitHub social preview / X / LinkedIn | `assets/launch/qarinah-social-preview.svg` and `.png` | 1280 x 640 | Main result-led launch banner |
...
Before Product Hunt, verify all three 1270 x 760 gallery images and the square 240 x 240 thumbnail at native size. Keep the benchmark card tied to the machine-readable result; never draw fictional token or cost dashboards.
...
**Thumbnail:** Green Q-shaped evidence path with linked nodes and a protected center on a dark background.

## Integrity rules

- use the existing Qarinah mark and palette;
- keep the full benchmark scope visible near the number;
- do not label estimated tokens as provider usage, billing, or cost savings;
- use real terminal output and machine-readable results;
- remove private paths, workspace data, credentials, and browser state; and
- caption video and provide useful alt text.
