...
| Native memory inside one commercial coding assistant | Copilot Memory, Claude Code memory, Cursor Memories, Windsurf Memories, Pieces, or Augment |
...
| [Claude Code memory](https://docs.anthropic.com/en/docs/claude-code/memory) | Hierarchical user, project, and local memory files loaded directly by Claude Code | Persistent project instructions and preferences | Typed event history, exact receipts, worktree comparison, conflicts, and reuse outside one host | Claude's native loading behavior, simple editable files, and zero-extra-tool onboarding |
| [Cursor Rules and Memories](https://docs.cursor.com/context/rules) | Low-friction memories and rules inside Cursor | Persistent coding context | Inspectable ledger, session receipts, conflicts, typed relations, and worktree comparison outside one editor | Native Cursor UI, automatic host-level memory creation, and deep editor integration |
