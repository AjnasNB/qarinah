...
test("session receipts bind exact session evidence without retaining event bodies", async (t) => {
  ...
  assert.equal(result.schemaVersion, "qarinah.session-context-receipt-index.v1");
  assert.equal(result.receiptCount, 2);
  const alpha = result.receipts.find((receipt) => receipt.sessionId === "session-alpha");
  assert.equal(alpha.source.eventCount, 2);
  ...
  assert.doesNotMatch(serialized, /PRIVATE_SESSION_BODY_ALPHA|PRIVATE_TOOL_RESULT_ALPHA|PRIVATE_SESSION_BODY_BETA/u);
  const stored = await readFile(path.join(root, ".qarinah", "receipts", "sessions", `${alpha.sessionKey}.json`), "utf8");
  assert.doesNotMatch(stored, /PRIVATE_SESSION_BODY_ALPHA|PRIVATE_TOOL_RESULT_ALPHA/u);
});
...
test("session receipt schema is closed and exposes only bounded measurement fields", async () => {
  ...
  assert.equal(schema.properties.schemaVersion.const, "qarinah.session-context-receipt.v1");
