#!/usr/bin/env python3
"""Build the separately versioned Qarinah v1.5 white-paper PDF."""
...
OUTPUT = ROOT / "output" / "pdf" / "Qarinah-Technical-White-Paper-v1.5.pdf"
...
SOURCE_INPUTS = (
    ...
    ("scripts/build-whitepaper-pdf-v1.5.py", Path(__file__).resolve()),
)
...
class WhitePaperV15DocTemplate(v14.WhitePaperV14DocTemplate):
    @staticmethod
    def draw_body_page(canvas, doc):
        v14.WhitePaperV14DocTemplate.draw_body_page(canvas, doc)
        canvas.saveState()
        canvas.setFillColor(legacy.LIGHT)
        canvas.rect(legacy.BODY_LEFT, 5.5 * legacy.mm, 65 * legacy.mm, 5 * legacy.mm, stroke=0, fill=1)
        canvas.setFillColor(legacy.MUTED)
        canvas.setFont(legacy.BODY_FONT, 7.2)
        canvas.drawString(legacy.BODY_LEFT, 8 * legacy.mm, "Ajnas N B - Technical white paper v1.5")
        canvas.restoreState()
...
def cover_story():
    ...
    replacements = {
        "Paper version 1.4 - August 2026": "Paper version 1.5 - August 2026",
        "Qarinah 0.1.6": "Qarinah 0.4.0",
        "The v1.4 version DOI is assigned at deposit. Concept DOI: 10.5281/zenodo.21547684.": (
            "Version 1.5 has no version DOI until a separate deposit is completed. "
...
def build():
    ...
    doc = WhitePaperV15DocTemplate(str(OUTPUT))
            ...
            json.dumps(
                {
                    ...
                    "generator": {
                        "command": "python scripts/build-whitepaper-pdf-v1.5.py",
    ...
    PDF_DIGEST.write_bytes(
        f"{pdf_digest}  output/pdf/Qarinah-Technical-White-Paper-v1.5.pdf\n".encode("ascii")
    )
    print(OUTPUT)
