#!/usr/bin/env python3
"""Build the separately versioned Qarinah v1.8 white-paper PDF."""
...
OUTPUT = ROOT / "output" / "pdf" / "Qarinah-Technical-White-Paper-v1.8.pdf"
...
SOURCE_INPUTS = (
    ...
    ("scripts/build-whitepaper-pdf-v1.8.py", Path(__file__).resolve()),
)
...
class WhitePaperV18DocTemplate(v17.WhitePaperV17DocTemplate):
    ...
    def draw_body_page(canvas, doc):
        ...
        canvas.drawString(legacy.BODY_LEFT, 8 * legacy.mm, "Ajnas N B - Technical white paper v1.8")
        canvas.restoreState()
...
def build():
    ...
    doc = WhitePaperV18DocTemplate(str(OUTPUT))
            ...
            json.dumps(
                {
                    ...
                    "generator": {
                        "command": "python scripts/build-whitepaper-pdf-v1.8.py",
    ...
    PDF_DIGEST.write_bytes(
        f"{pdf_digest}  output/pdf/Qarinah-Technical-White-Paper-v1.8.pdf\n".encode("ascii")
    )
    print(OUTPUT)
