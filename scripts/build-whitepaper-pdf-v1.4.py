#!/usr/bin/env python3
"""Build the separately versioned Qarinah v1.4 white-paper PDF."""
...
OUTPUT = ROOT / "output" / "pdf" / "Qarinah-Technical-White-Paper-v1.4.pdf"
SOURCE_DIGEST = ROOT / "output" / "pdf" / "Qarinah-Technical-White-Paper-v1.4.source.sha256"
PDF_DIGEST = ROOT / "output" / "pdf" / "Qarinah-Technical-White-Paper-v1.4.pdf.sha256"
BUILD_METADATA = ROOT / "output" / "pdf" / "Qarinah-Technical-White-Paper-v1.4.build.json"
SOURCE_INPUTS = (
    ("docs/WHITEPAPER.md", SOURCE),
    ("scripts/build-whitepaper-pdf.py", LEGACY_BUILDER),
    ("scripts/build-whitepaper-pdf-v1.4.py", Path(__file__).resolve()),
)
...
class WhitePaperV14DocTemplate(legacy.WhitePaperDocTemplate):
    ...
    def draw_body_page(canvas, doc):
        ...
        canvas.setFillColor(legacy.MUTED)
        canvas.setFont(legacy.BODY_FONT, 7.2)
        canvas.drawString(legacy.BODY_LEFT, 8 * legacy.mm, "Ajnas N B - Technical white paper v1.4")
...
def cover_story():
    ...
    return [
        ...
        legacy.Paragraph(
            "<b>Ajnas N B</b><br/>Paper version 1.4 - August 2026<br/>"
...
def compact_toc_story():
    toc = legacy.TableOfContents()
    toc.levelStyles = [
        legacy.ParagraphStyle(
            "TOC0V14",
            fontName=legacy.BOLD_FONT,
            fontSize=7.9,
            leading=9.2,
            leftIndent=0,
            firstLineIndent=0,
            textColor=legacy.INK,
            spaceBefore=0.35 * legacy.mm,
        ),
        legacy.ParagraphStyle(
            "TOC1V14",
            fontName=legacy.BODY_FONT,
            fontSize=6.8,
            leading=7.8,
            leftIndent=6 * legacy.mm,
            firstLineIndent=0,
            textColor=legacy.MUTED,
        ),
        legacy.ParagraphStyle(
            "TOC2V14",
            fontName=legacy.BODY_FONT,
            fontSize=6.8,
            leading=8.2,
            leftIndent=12 * legacy.mm,
            firstLineIndent=0,
            textColor=legacy.MUTED,
        ),
    ]
    return [
        legacy.Paragraph("Contents", legacy.STYLES["toc-title"]),
        legacy.Paragraph(
            "Implemented architecture, evidence model, security boundary, evaluation, and release status.",
            legacy.STYLES["body"],
        ),
        legacy.Spacer(1, 1.5 * legacy.mm),
        toc,
        legacy.PageBreak(),
    ]
...
def build():
    ...
    story = cover_story() + compact_toc_story() + legacy.parse_markdown(text[start:])
    doc = WhitePaperV14DocTemplate(str(OUTPUT))
    ...
    build_metadata = {
        ...
        "generator": {
            "command": "python scripts/build-whitepaper-pdf-v1.4.py",
    ...
    PDF_DIGEST.write_bytes(
        f"{pdf_digest}  output/pdf/Qarinah-Technical-White-Paper-v1.4.pdf\n".encode("ascii")
    )
    print(OUTPUT)
