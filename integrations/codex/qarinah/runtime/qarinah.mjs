#!/usr/bin/env node
// GENERATED by scripts/build-plugins.mjs for Qarinah 0.6.0. DO NOT EDIT.
...
function sanitizeJsonValue(value, options = {}) {
  ...
  let nodes = 0;
  function visit(candidate, path25, depth) {
    nodes += 1;
    if (nodes > limits.maximumNodes) throw new TypeError(`JSON value exceeds ${limits.maximumNodes} nodes.`);
    if (depth > limits.maximumDepth) throw new TypeError(`JSON value exceeds depth ${limits.maximumDepth} at ${path25}.`);
...
[QARINAH_TRUNCATED:${candidate.length - limits.maximumStringLength}]`;
    ...
    if (typeof candidate === "number") {
      if (!Number.isFinite(candidate)) throw new TypeError(`${path25} must contain only finite numbers.`);
      return Object.is(candidate, -0) ? 0 : candidate;
    }
    if (Array.isArray(candidate)) {
      if (candidate.length > limits.maximumArrayLength) {
        throw new TypeError(`${path25} exceeds ${limits.maximumArrayLength} array items.`);
      }
      ...
      for (const key of Reflect.ownKeys(descriptors2)) {
        if (typeof key !== "string" || !allowed.has(key)) throw new TypeError(`${path25} contains an unsupported array property.`);
      }
      return Array.from({ length: candidate.length }, (_, index) => {
        const descriptor = descriptors2[String(index)];
        if (!descriptor?.enumerable || !Object.hasOwn(descriptor, "value")) {
          throw new TypeError(`${path25}[${index}] must be an enumerable data property.`);
        }
        return visit(descriptor.value, `${path25}[${index}]`, depth + 1);
      });
    }
    if (!candidate || typeof candidate !== "object") throw new TypeError(`${path25} contains an unsupported value.`);
    const prototype = Object.getPrototypeOf(candidate);
    if (prototype !== Object.prototype && prototype !== null) {
      throw new TypeError(`${path25} must contain only plain records.`);
    }
    const descriptors = Object.getOwnPropertyDescriptors(candidate);
    const keys = Reflect.ownKeys(descriptors);
    if (keys.length > limits.maximumObjectKeys) throw new TypeError(`${path25} exceeds ${limits.maximumObjectKeys} fields.`);
    const result = /* @__PURE__ */ Object.create(null);
    for (const key of keys) {
      if (typeof key !== "string") throw new TypeError(`${path25} cannot contain symbol keys.`);
      const descriptor = descriptors[key];
      if (!descriptor.enumerable || !Object.hasOwn(descriptor, "value") || descriptor.value === void 0) {
        throw new TypeError(`${path25}.${key} must be an enumerable data property with a defined value.`);
      }
      result[key] = visit(descriptor.value, `${path25}.${key}`, depth + 1);
    }
    return result;
  }
...
function snapshotJsonBoundary(value, options = {}) {
  ...
  function visit(candidate, path25, depth) {
    ...
    if (depth > limits.maximumDepth) throw new TypeError(`${limits.label} exceeds depth ${limits.maximumDepth} at ${path25}.`);
    if (candidate === null || typeof candidate === "boolean") return candidate;
    if (typeof candidate === "string") {
      if (candidate.length > limits.maximumStringLength) {
        throw new TypeError(`${path25} exceeds ${limits.maximumStringLength} characters.`);
      }
      return candidate;
    }
    if (typeof candidate === "number") {
      if (!Number.isFinite(candidate) || Object.is(candidate, -0)) {
        throw new TypeError(`${path25} must be a finite JSON number other than negative zero.`);
      }
      return candidate;
    }
    if (!candidate || typeof candidate !== "object") throw new TypeError(`${path25} contains a non-JSON value.`);
    if (seen.has(candidate)) throw new TypeError(`${path25} contains a cyclic or repeated object reference.`);
    seen.add(candidate);
    if (Array.isArray(candidate)) {
      if (candidate.length > limits.maximumArrayLength) {
        throw new TypeError(`${path25} exceeds ${limits.maximumArrayLength} array entries.`);
      }
      const descriptors2 = Object.getOwnPropertyDescriptors(candidate);
      const expected = /* @__PURE__ */ new Set(["length", ...Array.from({ length: candidate.length }, (_, index) => String(index))]);
      for (const key of Reflect.ownKeys(descriptors2)) {
        if (typeof key !== "string" || !expected.has(key)) throw new TypeError(`${path25} contains an unsupported array property.`);
      }
      return Array.from({ length: candidate.length }, (_, index) => {
        const descriptor = descriptors2[String(index)];
        if (!descriptor?.enumerable || !Object.hasOwn(descriptor, "value")) {
    ...
    const prototype = Object.getPrototypeOf(candidate);
    if (prototype !== Object.prototype && prototype !== null) throw new TypeError(`${path25} must be a plain record.`);
    const descriptors = Object.getOwnPropertyDescriptors(candidate);
    const keys = Reflect.ownKeys(descriptors);
    if (keys.length > limits.maximumObjectKeys) throw new TypeError(`${path25} exceeds ${limits.maximumObjectKeys} fields.`);
    const result = /* @__PURE__ */ Object.create(null);
...
function validateStoredEvent(value, options = {}) {
  const event2 = record(value, "stored event", STORED_KEYS);
  if (event2.schemaVersion !== EVENT_SCHEMA_VERSION) throw new TypeError("Unsupported event schemaVersion.");
  const reconstructed = createEventEnvelope({
    eventId: event2.eventId,
    timestamp: event2.timestamp,
    sessionId: event2.sessionId,
    turnId: event2.turnId,
    kind: event2.kind,
    actor: event2.actor,
    title: event2.title,
    body: event2.body,
    ...
    ...event2.authority === void 0 ? {} : { authority: event2.authority },
    ...event2.temporal === void 0 ? {} : { temporal: event2.temporal },
    ...event2.repository === void 0 ? {} : { repository: event2.repository },
    ...event2.freshness === void 0 ? {} : { freshness: event2.freshness },
    ...event2.disclosure === void 0 ? {} : { disclosure: event2.disclosure },
    relations: event2.relations,
    provenance: event2.provenance,
    retention: event2.retention
  }, {
    workspaceId: event2.workspaceId,
    previousHash: event2.previousHash,
    maximumEventBytes: options.maximumEventBytes
  });
  if (event2.hash !== reconstructed.hash || !HASH_PATTERN.test(event2.hash)) {
    throw new TypeError(`Event '${event2.eventId}' hash does not match its canonical contents.`);
  }
  if (canonicalStringify(event2) !== canonicalStringify(reconstructed)) {
    throw new TypeError(`Event '${event2.eventId}' stored representation is not canonical.`);
  }
  if (options.expectedPreviousHash !== void 0 && event2.previousHash !== options.expectedPreviousHash) {
    throw new TypeError(`Event '${event2.eventId}' breaks hash-chain continuity.`);
  }
  if (options.workspaceId !== void 0 && event2.workspaceId !== options.workspaceId) {
    throw new TypeError(`Event '${event2.eventId}' belongs to a different workspace.`);
  }
...
var require_ignore = __commonJS({
  "node_modules/ignore/index.js"(exports, module) {
    ...
    var basenameOf = (path25) => {
      const end = path25.length - 1;
      const index = path25.lastIndexOf(
        SLASH,
        path25[end] === SLASH ? end - 1 : end
      );
      return index < 0 ? path25 : path25.slice(index + 1);
    };
    var parentOf = (path25) => {
      if (path25.charCodeAt(0) === SLASH_CODE || path25.indexOf(DOUBLE_SLASH) >= 0) {
        const slices = path25.split(SLASH).filter(Boolean);
        slices.pop();
        return slices.length ? slices.join(SLASH) + SLASH : EMPTY;
      }
      const end = path25.length - 1;
      const cut = path25.lastIndexOf(
        SLASH,
        path25.charCodeAt(end) === SLASH_CODE ? end - 1 : end
      );
      return cut < 0 ? EMPTY : path25.slice(0, cut + 1);
    };
    ...
    var RuleManager = class {
      ...
      test(path25, checkUnignored, mode) {
        ...
        const basename = shortcut ? basenameOf(path25) : path25;
        for (let index = 0; index < length; index++) {
          ...
          if (!skip && rule[mode].test(
            shortcut && rule._basenameOnly ? basename : path25
          )) {
            ignored = !negative;
            unignored = negative;
    ...
    var isNotRelative = (path25) => {
      const first = path25.charCodeAt(0);
      if (first === SLASH_CODE) {
        return true;
      }
      if (first !== DOT_CODE) {
      ...
      const second = path25.charCodeAt(1);
      ...
      return path25.length === 2 || path25.charCodeAt(2) === SLASH_CODE;
    };
    ...
    var Ignore = class {
      ...
      _test(originalPath, cache, checkUnignored) {
        ...
        return this._t(path25, cache, checkUnignored);
      }
      checkIgnore(path25) {
        if (path25.charCodeAt(path25.length - 1) !== SLASH_CODE) {
          return this.test(path25);
        }
        const parentPath = parentOf(path25);
        if (parentPath) {
      ...
      _t(path25, cache, checkUnignored) {
        if (path25 in cache) {
          return cache[path25];
        }
        const parentPath = parentOf(path25);
        const parent = parentPath ? this._t(parentPath, cache, checkUnignored) : UNDEFINED;
        return cache[path25] = parent && parent.ignored ? parent : this._rules.test(path25, checkUnignored, MODE_IGNORE);
      }
      ignores(path25) {
...
async function scanProjectStructure(rawOptions = {}) {
  ...
  const event2 = await appendEvent(eventInput, { workspace, capture: workspace.config.capture });
  return Object.freeze({
    captured: true,
    unchanged: false,
    eventId: event2.eventId,
...
function eventActiveAt(event2, asOf) {
  const validFrom = event2.temporal?.validFrom ?? event2.timestamp;
  if (event2.timestamp > asOf || validFrom > asOf) return false;
  if (event2.temporal?.validUntil !== null && event2.temporal?.validUntil !== void 0 && event2.temporal.validUntil <= asOf) return false;
  if (event2.retention?.expiresAt !== null && event2.retention?.expiresAt !== void 0 && event2.retention.expiresAt <= asOf) return false;
  return true;
}
function selectProjectionEvents(events, options) {
  ...
  return events.filter((event2) => {
    if (!eventActiveAt(event2, asOf)) return false;
    if (event2.disclosure?.classification === "restricted" && !(event2.disclosure.scopes ?? []).some((scope) => selectors.authorityScopes.includes(scope))) return false;
    return selectors.repositoryIds.length === 0 || event2.repository?.id === void 0 || event2.repository?.id === null || selectors.repositoryIds.includes(event2.repository.id);
  });
}
function buildLinkedProjectMemory(events, workspaceId, options = {}) {
  ...
  const sourceEventIds = new Set(sourceEvents.map((event2) => event2.eventId));
  const currentStructure = latestProjectStructure2(sourceEvents);
  const sourcePositions = new Map(sourceEvents.map((event2, index) => [event2.eventId, index]));
  let projectedEvents = sourceEvents.slice(-MAX_GRAPH_EVENTS);
  if (currentStructure && !projectedEvents.some((event2) => event2.eventId === currentStructure.event.eventId)) {
  ...
  const projectedEventIds = new Set(projectedEvents.map((event2) => event2.eventId));
  const sourceRelationCount = sourceEvents.reduce((sum, event2) => sum + event2.relations.length, 0);
  ...
  for (let index = projectedEvents.length - 1; index >= 0; index -= 1) {
    const event2 = projectedEvents[index];
    const relations = [];
    for (const relation of event2.relations) {
    ...
    selectedRelations.set(event2.eventId, relations);
  }
  projectedEvents = projectedEvents.map((event2) => {
    const relations = selectedRelations.get(event2.eventId);
    return relations.length === event2.relations.length ? event2 : { ...event2, relations };
  });
  ...
  const relationCount = events.reduce((sum, event2) => sum + event2.relations.length, 0);
  ...
  for (const event2 of events) {
    const node = {
      ...
      language: null,
      timestamp: event2.timestamp,
      validFrom: event2.temporal?.validFrom ?? event2.timestamp,
      validUntil: event2.temporal?.validUntil ?? null,
      expiresAt: event2.retention?.expiresAt ?? null,
      confidence: event2.confidence ?? "extracted",
      status: superseded.has(event2.eventId) ? "superseded" : "current",
      supersededBy: [...supersededBy.get(event2.eventId) ?? []].sort(),
      conflicted: conflicted.has(event2.eventId),
      repositoryId: event2.repository?.id ?? null,
      disclosureScopes: event2.disclosure?.scopes ?? [],
      classification: event2.disclosure?.classification ?? "workspace",
      sourceProfiles: [],
      sourceEventId: event2.eventId,
      evidenceHash: event2.hash,
      contentHash: event2.provenance?.contentHash ?? null,
      searchText: [event2.title, event2.body, ...dataText(event2.data)].join("\n")
    };
    nodes.push(node);
  ...
  for (const event2 of events) {
    for (const relation of event2.relations) {
      pendingRelations.push({ event: event2, relation });
    }
  }
  ...
  if (currentStructure) {
    const { event: event2 } = currentStructure;
  ...
  for (const [target, entries] of pendingByTarget) {
    ...
    const profiles = entries.map(({ event: event2 }) => ({
      sourceNodeId: event2.eventId,
      sourceEventId: event2.eventId,
      evidenceHash: event2.hash,
      contentHash: event2.provenance?.contentHash ?? null,
      classification: event2.disclosure?.classification ?? "workspace",
      disclosureScopes: [...event2.disclosure?.scopes ?? []].sort(),
      repositoryId: event2.repository?.id ?? null,
      validFrom: event2.temporal?.validFrom ?? event2.timestamp,
      validUntil: event2.temporal?.validUntil ?? null,
      expiresAt: event2.retention?.expiresAt ?? null
  ...
  for (const { event: event2, relation } of pendingRelations) {
  ...
  const eventTimes = events.map((event2) => Date.parse(event2.timestamp));
  ...
  const core = {
    ...
    timeline: timeline2.map((event2) => event2.eventId),
    repositoryMap: {
...
function toolName(event2) {
  return boundedText(event2.data?.toolName, boundedText(event2.title, "tool"));
}
function eventEvidence(event2) {
  return {
    eventId: event2.eventId,
    hash: event2.hash,
    timestamp: event2.timestamp,
    sourceId: event2.provenance.sourceId
  };
}
...
function decisionRecord(event2, tools, superseded) {
  const explicitlyRelated = new Set(event2.relations.map((relation) => relation.target));
  return {
    ...eventEvidence(event2),
    title: event2.title,
    status: superseded.has(event2.eventId) ? "superseded" : "current",
    reason: boundedText(event2.data?.reason, boundedText(event2.body, "No reason was recorded.")),
    outcome: boundedText(event2.data?.outcome),
    alternatives: Array.isArray(event2.data?.alternatives) ? event2.data.alternatives.filter((value) => typeof value === "string").slice(0, 20).map((value) => boundedText(value)) : [],
    affected: event2.relations.filter((relation) => relation.type === "affects" || relation.type === "changed").map((relation) => relation.target),
    tools: relatedTools(event2, tools, explicitlyRelated)
  };
}
function flowStep(event2, sequence) {
  return {
    sequence,
    ...eventEvidence(event2),
    sessionId: event2.sessionId,
    ...
    actor: event2.actor,
    title: event2.title,
    detail: boundedText(event2.body),
    toolName: event2.kind === "tool.requested" || event2.kind === "tool.completed" ? toolName(event2) : null
  };
}
...
function majorChange(event2) {
  const affected = event2.relations.filter((relation) => ["affects", "changed", "produced"].includes(relation.type)).map((relation) => relation.target);
  return {
    ...eventEvidence(event2),
    kind: event2.kind,
    title: event2.title,
    summary: boundedText(event2.data?.outcome, boundedText(event2.body)),
    affected
  };
}
function buildProjectRecordViews(events, workspaceId) {
  const byId = new Map(events.map((event2) => [event2.eventId, event2]));
  const superseded = /* @__PURE__ */ new Set();
  for (const event2 of events) {
    for (const relation of event2.relations) {
  ...
  const tools = events.filter((event2) => event2.kind === "tool.requested" || event2.kind === "tool.completed");
  const decisions = events.filter((event2) => event2.kind === "decision").slice(-MAX_DECISIONS).map((event2) => decisionRecord(event2, tools, superseded));
  const flowEvents = events.filter((event2) => [
    "session.started",
    "prompt.submitted",
    "tool.requested",
    "tool.completed",
    ...
    "compaction.started",
    "compaction.completed"
  ].includes(event2.kind)).slice(-MAX_FLOW_STEPS);
  ...
  return deepFreezeJson({
    ...
    majorChanges: events.filter((event2) => event2.kind === "decision" || event2.kind === "artifact" || event2.kind === "turn.completed").slice(-MAX_MAJOR_CHANGES).map(majorChange),
...
function insertEvents(database, events) {
  ...
  for (const event2 of events) {
    const validFrom = event2.temporal?.validFrom ?? event2.timestamp;
    const validUntil = event2.temporal?.validUntil ?? null;
    insertEvent.run(
      event2.eventId,
      event2.workspaceId,
      event2.timestamp,
      ...
      event2.title,
      event2.body,
      canonical(event2.data),
      event2.confidence,
      event2.authority?.scope ?? null,
      event2.repository?.id ?? null,
      validFrom,
      validUntil,
      event2.hash
    );
    insertFts.run(event2.eventId, event2.title, event2.body, primitiveDataText(event2.data).join("\n"));
    insertCitation.run(event2.eventId, event2.provenance.sourceId, event2.provenance.adapter, event2.provenance.contentHash);
    if (event2.provenance.sourceId) {
      insertSource.run(event2.provenance.sourceId, event2.provenance.adapter, event2.eventId, event2.provenance.contentHash);
    }
    if (event2.kind === "decision") insertDecision.run(event2.eventId, "current", validFrom, validUntil);
    for (const file of event2.freshness?.files ?? []) {
      insertFreshness.run(event2.eventId, "file", file.path, file.hash, event2.repository?.branch ?? null, event2.repository?.commit ?? null);
    }
    for (const dependency of event2.freshness?.dependencies ?? []) {
      insertFreshness.run(event2.eventId, "dependency", dependency.name, dependency.hash, event2.repository?.branch ?? null, event2.repository?.commit ?? null);
    }
    const disclosure = event2.data?.memoryAttachment;
    if (["memory.scope.attached", "memory.scope.revoked"].includes(event2.kind) && disclosure && typeof disclosure === "object") {
      insertDisclosure.run(
        ...
        disclosure.attachedAt ?? event2.timestamp,
        disclosure.expiresAt ?? null,
        event2.kind === "memory.scope.revoked" ? disclosure.revokedAt ?? event2.timestamp : null,
        disclosure.assignedBy ?? event2.actor.id,
        event2.eventId
      );
    }
    const pack = event2.data?.contextPack;
    if (event2.kind === "context.pack.compiled" && pack?.manifestHash) {
      insertPack.run(
        pack.manifestHash,
        event2.eventId,
        pack.query ?? "",
        event2.timestamp,
        pack.asOf ?? event2.timestamp,
        canonical(pack)
      );
...
async function rebuildSqliteReadModel(workspace, events, derived) {
  ...
  try {
    ...
    try {
      ...
      insertGraph(database, derived.graph, new Map(events.map((event2) => [event2.eventId, event2])));
...
function searchableText(event2) {
  const selectedData = [];
  for (const [key, value] of Object.entries(event2.data)) {
    if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") {
      selectedData.push(`${key} ${value}`);
    }
  }
  const structure = event2.data?.projectStructure;
  if (validateProjectStructureSnapshot(structure)) {
    for (const file of structure.files) {
...
function eventProjection(event2) {
  const searchable = searchableText(event2);
  ...
  return Object.freeze({
    ...
    data: event2.data,
    confidence: event2.confidence,
    authority: event2.authority ?? null,
    temporal: event2.temporal ?? null,
    repository: event2.repository ?? null,
    freshness: event2.freshness ?? null,
    disclosure: event2.disclosure ?? null,
    relations: event2.relations,
    provenance: event2.provenance,
    retention: event2.retention,
    hash: event2.hash,
    terms: [...new Set(eventLexemes)].sort(),
    titleTerms: tokenize(event2.title),
...
function buildDerivedState(events, workspaceId) {
  ...
  const nodes = [];
  const edges = [];
  for (const event2 of projections) {
    nodes.push({
      id: event2.eventId,
      type: event2.kind,
      timestamp: event2.timestamp,
    ...
    adjacency[event2.eventId] = [];
    for (const term of event2.terms) {
      if (!postings[term]) postings[term] = [];
      postings[term].push(event2.eventId);
      documentFrequency[term] = (documentFrequency[term] || 0) + 1;
    }
    for (const relation of event2.relations) {
      adjacency[event2.eventId].push({ type: relation.type, target: relation.target });
      edges.push({ source: event2.eventId, type: relation.type, target: relation.target });
    }
  }
  ...
  const averageDocumentLength = projections.length === 0 ? 0 : projections.reduce((total, event2) => total + event2.documentLength, 0) / projections.length;
  const index = {
...
function markdownFor(events, workspaceId, headHash) {
  ...
  for (const event2 of selected3) {
    lines.push(`### ${markdownInline(event2.title)}`);
    lines.push("");
    lines.push(`- ID: \`${event2.eventId}\``);
    lines.push(`- Kind: \`${event2.kind}\``);
    lines.push(`- Time: ${event2.timestamp}`);
    lines.push(`- Confidence: \`${event2.confidence}\``);
    lines.push(`- Hash: \`${event2.hash}\``);
    if (event2.body) {
      lines.push("");
      const body = event2.body.length > 1e3 ? `${event2.body.slice(0, 997)}...` : event2.body;
...
function indexedEventEntry(event2, offset, length) {
  return Object.freeze({ hash: event2.hash, offset, length });
}
function indexEventLocations(events, logBytes) {
  const locations = [];
  let offset = 0;
  for (const event2 of events) {
    const length = Buffer.byteLength(`${canonicalStringify(event2)}
`);
    locations.push(Object.freeze({ event: event2, entry: indexedEventEntry(event2, offset, length) }));
    offset += length;
  }
  if (offset !== logBytes) {
...
async function writeEventIdProjection(workspace, events, logBytes) {
  ...
  for (const { event: event2, entry } of indexEventLocations(events, logBytes)) {
    const bucketName = eventIdBucket(event2.eventId);
    if (!groups.has(bucketName)) groups.set(bucketName, /* @__PURE__ */ Object.create(null));
    groups.get(bucketName)[event2.eventId] = entry;
  }
...
async function appendEventIdProjection(workspace, trusted, event2, location, options) {
  const directories = await ensureEventIdIndexDirectories(workspace);
  const bucketName = eventIdBucket(event2.eventId);
  const current = await loadEventIdBucket(workspace, trusted.manifest, bucketName);
  if (Object.hasOwn(current.entries, event2.eventId)) {
    throw new QarinahError("EVENT_ID_DUPLICATE", `Event id '${event2.eventId}' already exists in this workspace.`);
  }
  const entries = { ...current.entries, [event2.eventId]: indexedEventEntry(event2, location.offset, location.length) };
  const bucket = {
    schemaVersion: EVENT_ID_BUCKET_SCHEMA_VERSION,
    workspaceId: workspace.config.workspaceId,
    bucket: bucketName,
...
`, {
    sync: false,
    afterWrite: () => injectStoreFault(options, "after-event-id-bucket-write", { bucketName, eventId: event2.eventId }),
    afterRename: () => injectStoreFault(options, "after-event-id-bucket-rename", { bucketName, eventId: event2.eventId })
  });
  const manifest = {
    ...trusted.manifest,
    eventCount: trusted.manifest.eventCount + 1,
    headHash: event2.hash,
...
`, {
    sync: false,
    afterWrite: () => injectStoreFault(options, "after-event-id-manifest-write", { eventId: event2.eventId }),
    afterRename: () => injectStoreFault(options, "after-event-id-manifest-rename", { eventId: event2.eventId })
  });
  return Object.freeze({ manifest, manifestHash: sha256(manifest) });
}
async function readIndexedEvent(workspace, eventId, entry) {
  ...
  let event2;
  try {
    event2 = validateStoredEvent(parsed, {
      workspaceId: workspace.config.workspaceId,
  ...
  if (event2.eventId !== eventId || event2.hash !== entry.hash) {
...
async function readHeadFromHandle(workspace, handle, metadata) {
  ...
  try {
    const event2 = validateStoredEvent(parsed, {
      workspaceId: workspace.config.workspaceId,
      maximumEventBytes: workspace.config.maxEventBytes
    });
    return Object.freeze({ event: event2, size: metadata.size, dev: metadata.dev, ino: metadata.ino });
  } catch (error) {
...
async function readEventsFromWorkspace(workspace, options = {}) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    try {
      ...
      if (eventIds.has(event2.eventId)) throw new TypeError(`Event id '${event2.eventId}' is duplicated.`);
...
function sourceEventDescriptor(event2) {
  const { schemaVersion: _schemaVersion, workspaceId: _workspaceId, previousHash: _previousHash, hash: _hash, ...source } = event2;
  return source;
}
...
async function appendEvent(input, options = {}) {
  ...
  try {
    ...
    let event2 = createCapturedEvent(input, workspace, previousHash, options, capture, reviewedMetadata);
    let existing;
    try {
      existing = await loadIndexedEvent(workspace, trustedProjection.manifest, event2.eventId);
    } catch (error) {
      if (!["ENOENT", "EVENT_ID_INDEX_INVALID", "EVENT_ID_INDEX_MISMATCH"].includes(error?.code)) throw error;
      throwIfAborted(signal);
      ({ head, consent, trustedProjection } = await rebuildTrustedEventIdProjection(workspace));
      event2 = createCapturedEvent(
        { ...input, eventId: event2.eventId, timestamp: event2.timestamp },
        workspace,
    ...
    if (existing && options.idempotent === true) {
      ...
      throw new QarinahError("EVENT_ID_CONFLICT", `Event id '${event2.eventId}' was reused with different content.`);
    }
    if (existing) {
      throw new QarinahError("EVENT_ID_DUPLICATE", `Event id '${event2.eventId}' already exists in this workspace.`);
    }
    const line = `${canonicalStringify(event2)}
`;
    ...
    try {
      ...
      if (finalHead.event?.hash !== event2.hash) {
    ...
    await injectStoreFault(options, "after-checkpoint-update", { eventId: event2.eventId });
    return event2;
  } finally {
    await release();
  }
...
function eventSearchText(event2) {
  return normalizedText(`${event2.title}
${event2.body}
${primitiveDataText2(event2.data)}`);
}
...
function bm25Scores(index, queryTerms, eligibleEventIds) {
  ...
  const k1 = 1.2;
  const b = 0.75;
  for (const event2 of index.events) {
    if (!eligibleEventIds.has(event2.eventId)) continue;
    let score = 0;
    for (const term of queryTerms) {
      const frequency = event2.termFrequencies?.[term] || 0;
      ...
      const denominator = frequency + k1 * (1 - b + b * ((event2.documentLength || 0) / averageLength));
      const titleBoost = event2.titleTerms?.includes(term) ? 1.8 : 1;
      score += inverseFrequency * (frequency * (k1 + 1) / denominator) * titleBoost;
    }
    if (score > 0) scores.set(event2.eventId, rounded(score));
  }
  return scores;
}
function fuzzyScores(index, query, eligibleEventIds) {
  ...
  for (const event2 of index.events) {
    if (!eligibleEventIds.has(event2.eventId)) continue;
    const similarity = jaccard(queryGrams, ngrams(eventSearchText(event2)));
    const phraseBonus = eventSearchText(event2).includes(normalizedQuery) ? 0.5 : 0;
    const score = similarity + phraseBonus;
    if (score >= 0.025) scores.set(event2.eventId, rounded(score));
  }
  return scores;
}
...
function authorityScores(index, candidateIds, scopes, asOf) {
  ...
  for (const event2 of index.events) {
    if (!candidateIds.has(event2.eventId)) continue;
    const authority = event2.authority;
    ...
    scores.set(event2.eventId, 1 + authority.rank / 100);
  }
  return scores;
}
...
function permittedByDisclosure(event2, scopes) {
  const disclosure = event2.disclosure;
  if (!disclosure || disclosure.classification !== "restricted") return true;
  return disclosure.scopes.some((scope) => scopes.includes(scope));
}
function permittedRepository(event2, repositoryIds) {
  return repositoryIds.length === 0 || event2.repository === null || event2.repository !== void 0 && repositoryIds.includes(event2.repository.id);
}
...
function resolveContextAdmission(index, options = {}) {
  ...
  for (const event2 of index.events) {
    const reasons = [];
    if (event2.retention?.expiresAt !== null && event2.retention?.expiresAt <= asOf) {
      reasons.push("expired");
      filters.expired += 1;
    }
    if (temporalBoundary === "strict-before" ? event2.timestamp >= asOf : event2.timestamp > asOf) {
      reasons.push("future");
      filters.future += 1;
    }
    if (event2.temporal?.validFrom !== void 0 && event2.temporal.validFrom > asOf) {
      reasons.push("not-yet-valid");
      filters.notYetValid += 1;
    }
    if (event2.temporal?.validUntil !== null && event2.temporal?.validUntil !== void 0 && event2.temporal.validUntil <= asOf) {
      reasons.push("stale");
      filters.stale += 1;
    }
    const disclosurePermitted = permittedByDisclosure(event2, authorityScopes);
    const repositoryPermitted = permittedRepository(event2, repositoryIds);
    ...
    if (reasons.length === 0) {
      eligibleEventIds.push(event2.eventId);
      continue;
    }
    excludedEventIds.push(event2.eventId);
    exclusions.push(Object.freeze({ eventId: event2.eventId, reasons: Object.freeze(reasons) }));
  }
  return Object.freeze({
    asOf,
    temporalBoundary,
...
function resolveCurrentContextState(index, orderedEventIds, options = {}) {
  ...
  const allEventsById = new Map(index.events.map((event2) => [event2.eventId, event2]));
...
function evidenceSufficiency(index, query, queryTerms, eligibleEventIds, lexical, fuzzy) {
  ...
  for (const event2 of index.events) {
    if (!eligibleEventIds.has(event2.eventId)) continue;
    const matchedTerms = queryTerms.filter((term) => (event2.termFrequencies?.[term] || 0) > 0).length;
    const termRatio = matchedTerms / queryTerms.length;
    const matchedEntities = codeEntities.filter((term) => (event2.termFrequencies?.[term] || 0) > 0).length;
    ...
    if (termRatio > bestExactTermRatio || termRatio === bestExactTermRatio && matchedEntities > bestMatchedCodeEntities) {
      ...
      bestConfidence = { verified: 1, claimed: 0.8, extracted: 0.7, inferred: 0.5 }[event2.confidence] ?? 0.5;
    }
  }
...
function queryCoverage(index, queryTerms, eligibleEventIds, lexical, fuzzy) {
  ...
  for (const event2 of index.events) {
    if (!eligibleEventIds.has(event2.eventId)) continue;
    const matched = queryTerms.filter((term) => (event2.termFrequencies?.[term] || 0) > 0).length;
...
function rankContextEvents(index, query = "", options = {}) {
  ...
  if (queryTerms.length === 0) {
    const newest = [...eventsById.values()].sort((left, right) => right.timestamp.localeCompare(left.timestamp) || left.eventId.localeCompare(right.eventId)).map((event2, index2) => ({ event: event2, score: 1 / (index2 + 1), components: [{ name: "recency", rank: index2 + 1 }] }));
...
function projectStructureExcerpt(event2, query, maximum) {
  const structure = event2.data?.projectStructure;
  if (!validateProjectStructureSnapshot(structure)) {
    return null;
  }
...
function excerptFor(event2, query = "", maximum = 2e3) {
  const structureExcerpt = projectStructureExcerpt(event2, query, maximum);
  if (structureExcerpt !== null) return structureExcerpt;
  const pieces = [event2.body, compactData(event2.data)].filter(Boolean);
...
function createContextHandoffCapsule(pack, events, options = {}) {
  ...
  const event2 = validateStoredEvent(stored, { workspaceId: pack.workspaceId });
  if (event2.hash !== item.hash || event2.title !== item.title || event2.kind !== item.kind) {
    throw new TypeError("Selected context item does not match the verified handoff event.");
  }
  const sources = event2.data?.sourceEvents;
  ...
  for (const [index, source] of sources.entries()) {
    ...
    if (!event2.relations.some((relation) => relation.type === "derived_from" && relation.target === source.eventId)) {
  ...
  const footer = [
    `event ${event2.eventId} ${event2.hash}`,
    `pack ${pack.manifestHash}`,
    `confidence ${event2.confidence}; sources ${sources.length}`
  ];
  ...
  const fullTitle = capsuleInline(event2.title);
  const fullBody = capsuleInline(event2.body);
...
async function consolidateProjectFacts(options = {}) {
  ...
  const sourceEvents = events.filter((event2) => event2.provenance?.adapter !== "qarinah-fact-consolidation");
  ...
  if (normalized.record && facts.length > 0) {
    ...
    const existing = events.find((event3) => event3.eventId === eventId);
    if (existing !== void 0) {
    ...
    const event2 = await appendEvent(input, { workspace, capture: workspace.config.capture, idempotent: true, signal: normalized.signal });
    if (normalized.rebuild) await rebuildDerivedState(workspace.root, { signal: normalized.signal });
    recording = Object.freeze({ status: "recorded", eventId: event2.eventId, hash: event2.hash });
  }
  return deepFreezeJson({ ...core, manifestHash, recording });
}
...
function lifecycleForFact(fact, eventById, incoming, asOf) {
  const sourceStates = fact.sourceEventIds.map((eventId) => {
    const event2 = eventById.get(eventId);
    ...
    const contradicting = [
      ...
      ...(event2?.relations ?? []).filter((relation) => relation.type === "contradicts").map((relation) => eventById.get(relation.target)).filter(Boolean)
    ...
    const expiredAt = event2?.retention?.expiresAt ?? event2?.temporal?.validUntil ?? null;
    ...
    return {
      eventId,
      eventHash: event2?.hash ?? null,
      status: status2,
      validFrom: event2?.temporal?.validFrom ?? event2?.timestamp ?? null,
...
function incomingRelations(events) {
  ...
  for (const event2 of events) {
    for (const relation of event2.relations) {
      ...
      value[relation.type].push(event2);
...
function proofBase(normalized, workspace, pack, facts, events, symbols) {
  const eventById = new Map(events.map((event2) => [event2.eventId, event2]));
  ...
  const excludedSources = (pack.retrieval.exclusions ?? []).map((exclusion) => {
    const event2 = eventById.get(exclusion.eventId);
    ...
    return {
      ...
      eventHash: event2?.hash ?? null,
      title: event2?.title ?? "Unavailable excluded event",
      reason: exclusion.reason,
      validFrom: event2?.temporal?.validFrom ?? event2?.timestamp ?? null,
      validUntil: replacements[0]?.timestamp ?? event2?.temporal?.validUntil ?? event2?.retention?.expiresAt ?? null,
...
function existingHarnessRecord(events, key) {
  return [...events].reverse().find((event2) => event2.provenance?.adapter === HARNESS_ADAPTER && event2.data?.codingHarness?.runKey === key) ?? null;
}
function latestHarnessCheckpoint(events, options, worktree) {
  return [...events].reverse().find((event2) => event2.provenance?.adapter === HARNESS_ADAPTER && event2.data?.codingHarness?.query === options.query && (event2.data?.codingHarness?.worktreeId ?? null) === (worktree?.worktreeId ?? null) && (event2.data?.codingHarness?.repositoryId ?? null) === (worktree?.repositoryId ?? null)) ?? null;
}
function incrementalState(sourceEvents, sourceHeadHash, previous) {
  ...
  const previousIndex = sourceEvents.findIndex((event2) => event2.hash === previousSourceHeadHash);
  return Object.freeze({
...
async function recordHarnessSummary(options, workspace, worktree, pack, events, summary, key, sourceHeadHash, metrics, incremental) {
  ...
  const input = workspace.config.capture === "metadata" ? reviewMetadataEventInput(payload) : payload;
  const event2 = await appendEvent(input, {
    workspace,
    capture: workspace.config.capture,
    idempotent: true,
    signal: options.signal
  ...
  return Object.freeze({ status: existingHarnessRecord(events, key) ? "reused" : "created", eventId: event2.eventId, hash: event2.hash });
}
async function compileWorktree(options, descriptor, isCurrent) {
  ...
  const sourceEvents = events.filter((event2) => event2.provenance?.adapter !== HARNESS_ADAPTER);
  ...
  const ledgerCharacters = sourceEvents.reduce((total, event2) => total + canonicalStringify(event2).length + 1, 0);
...
async function importAgentArchive(source, options = {}) {
  ...
  const existingEventIds = new Set((await readEvents(workspace)).map((event2) => event2.eventId));
  let recordsSeen = 0;
  let visibleItems = 0;
...
`]),
    ...
    if (options.cwd !== void 0) {
      ...
      event2 = await appendEvent(reviewMetadataEventInput({
        kind: "artifact",
...
function importedSourceBytes(events) {
  ...
  for (const event2 of events) {
    const receipt = event2.data?.archiveImport;
...
async function measureMemoryFootprint(options = {}) {
  ...
  const workspace = await loadWorkspace(options.cwd ?? process.cwd());
  const events = await readEvents(workspace, { updateCheckpoint: options.updateCheckpoint !== false });
  const ledgerCharacters = events.reduce((total, event2) => total + canonicalStringify(event2).length + 1, 0);
...
function outcome(event2) {
  const body = event2.body.replaceAll(/\s+/gu, " ").trim();
  return {
    eventId: event2.eventId,
    hash: event2.hash,
    kind: event2.kind,
...
async function buildProjectOverview(options = {}) {
  ...
  const sessions = new Set(events.map((event2) => event2.sessionId).filter(Boolean));
  const files = latest?.structure.files ?? [];
  const references = files.flatMap((file) => file.references ?? []);
  const outcomes = events.filter((event2) => ["turn.completed", "decision", "summary", "approval"].includes(event2.kind)).slice(-maxOutcomes).reverse().map(outcome);
  return deepFreezeJson({
    ...
    memory: {
      sessions: sessions.size,
      prompts: events.filter((event2) => event2.kind === "prompt.submitted").length,
      toolRequests: events.filter((event2) => event2.kind === "tool.requested").length,
      toolOutcomes: events.filter((event2) => event2.kind === "tool.completed").length,
      completedTurns: events.filter((event2) => event2.kind === "turn.completed").length,
      decisions: events.filter((event2) => event2.kind === "decision").length,
      summaries: events.filter((event2) => event2.kind === "summary").length,
      approvals: events.filter((event2) => event2.kind === "approval").length,
...
function citationsFor(event2) {
  ...
  if (event2.data?.citation) add(event2.data.citation);
  if (Array.isArray(event2.data?.citations)) {
    for (const citation of event2.data.citations) add(citation);
  }
  add({
    url: event2.data?.canonicalUrl,
    title: event2.data?.title,
    author: event2.data?.author,
    publishedAt: event2.data?.publishedAt
  });
  for (const relation of event2.relations) {
    if (relation.type === "references") add({ url: relation.target });
  }
  add({ url: event2.provenance.sourceId });
...
function sortedRelations(event2) {
  return [...event2.relations].sort((left, right) => compareText(left.type, right.type) || compareText(left.target, right.target));
}
function prettyCanonicalJson(value) {
  return JSON.stringify(JSON.parse(canonicalStringify(value)), null, 2);
}
function renderEventConcept(event2, eventById) {
  const citations = citationsFor(event2);
  const relations = sortedRelations(event2);
  const description = safeOneLine(event2.body, `Recorded ${event2.kind} event.`, 240);
  const metadata = [
    ["type", "Qarinah Event"],
    ["title", safeOneLine(event2.title, event2.eventId, 512)],
    ["description", description],
    ["resource", `qarinah:event:${event2.eventId}`],
    ["tags", ["qarinah", "event", event2.kind, event2.actor.type, event2.confidence]],
    ["timestamp", event2.timestamp],
    ["qarinah_schema_version", "qarinah.okf-event.v1"],
    ["qarinah_derived", true],
    ["qarinah_workspace_id", event2.workspaceId],
    ["qarinah_event_id", event2.eventId],
    ["qarinah_event_schema_version", event2.schemaVersion],
    ["qarinah_event_hash", event2.hash],
    ["qarinah_previous_hash", event2.previousHash],
    ["qarinah_kind", event2.kind],
    ["qarinah_confidence", event2.confidence],
    ["qarinah_authority", event2.authority ?? null],
    ["qarinah_actor", event2.actor],
    ["qarinah_session_id", event2.sessionId],
    ["qarinah_turn_id", event2.turnId],
    ["qarinah_relations", relations],
    ["qarinah_provenance", {
      adapter: event2.provenance.adapter,
      content_hash: event2.provenance.contentHash,
      source_id: event2.provenance.sourceId
    }],
    ["qarinah_citations", citations],
    ["qarinah_retention", event2.retention]
  ];
  const lines = [
    frontmatter(metadata),
    `# ${markdownInline(event2.title)}`,
    "",
    ...
    "## Event",
    "",
    `- Event ID: \`${event2.eventId}\``,
    `- Kind: \`${event2.kind}\``,
    `- Timestamp: \`${event2.timestamp}\``,
    `- Actor: ${markdownInline(`${event2.actor.type}:${event2.actor.id}`)}`,
    `- Confidence: \`${event2.confidence}\``,
    `- Event hash: \`${event2.hash}\``,
    `- Previous hash: ${event2.previousHash === null ? "none" : `\`${event2.previousHash}\``}`,
    "",
    "## Recorded body (untrusted evidence)",
    "",
    markdownDataBlock(event2.body || "(empty)"),
    "",
    "## Data (untrusted evidence)",
    "",
    markdownDataBlock(prettyCanonicalJson(event2.data)),
    "",
    "## Relations",
    ""
  ];
  ...
  lines.push(
    ...
    "## Provenance",
    "",
    `- Adapter: ${markdownInline(event2.provenance.adapter)}`,
    `- Source ID: ${event2.provenance.sourceId === null ? "none" : markdownInline(event2.provenance.sourceId)}`,
    `- Content hash: \`${event2.provenance.contentHash}\``
  );
...
function renderIndex(events, workspaceId, headHash) {
  ...
  for (const event2 of [...events].sort(newestFirst)) {
    const description = safeOneLine(event2.body, `${event2.kind} at ${event2.timestamp}.`, 160);
    lines.push(`* [${markdownInline(event2.title)}](events/${event2.eventId}.md) - ${markdownInline(description)}`);
  }
  return `${lines.join("\n")}
`;
}
function renderLog(events) {
  ...
  let date = null;
  for (const event2 of [...events].sort(newestFirst)) {
    const nextDate = event2.timestamp.slice(0, 10);
    if (nextDate !== date) {
      if (date !== null) lines.push("");
      lines.push(`## ${nextDate}`, "");
      date = nextDate;
    }
    lines.push(`* **Recorded**: [${markdownInline(event2.title)}](events/${event2.eventId}.md) - \`${event2.kind}\` at \`${event2.timestamp}\`; hash \`${event2.hash}\`.`);
  }
  if (events.length === 0) lines.push("_No events have been recorded._");
  return `${lines.join("\n")}
`;
...
function buildBundle(events, workspaceId) {
  ...
  for (const event2 of [...events].sort((left, right) => compareText(left.eventId, right.eventId))) {
    files.set(`events/${event2.eventId}.md`, renderEventConcept(event2, eventById));
  }
...
async function captureCodexHook(input, options = {}) {
  ...
  const eventInput = workspace.config.capture === "metadata" ? reviewMetadataEventInput(payload) : payload;
  const event2 = await appendEvent(eventInput, {
    workspace,
    capture: workspace.config.capture,
    idempotent: Object.hasOwn(payload, "eventId")
  });
  return Object.freeze({ captured: true, eventId: event2.eventId, hash: event2.hash });
}
...
// src/setup.js
init_errors();
import { lstat as lstat13, mkdir as mkdir8, readFile as readFile8 } from "node:fs/promises";
...
async function buildReceipt(workspace, sessionId2, events, options) {
  ...
  const receiptQuery = options.query ?? events.map((event2) => event2.title).join(" ").slice(0, 4096);
  ...
  const sourceCharacters = events.reduce((total, event2) => total + canonicalStringify(event2).length + 1, 0);
  ...
  const toolRequests = events.filter((event2) => event2.kind === "tool.requested").length;
  const toolOutcomes = events.filter((event2) => event2.kind === "tool.completed").length;
  const turnIds = [...new Set(events.map((event2) => event2.turnId).filter((value) => typeof value === "string"))].sort();
  const kindCounts = [...events.reduce((counts, event2) => counts.set(event2.kind, (counts.get(event2.kind) ?? 0) + 1), /* @__PURE__ */ new Map())].map(([kind, count]) => ({ kind, count })).sort((left, right) => left.kind.localeCompare(right.kind));
  const completedTurns = events.filter((event2) => event2.kind === "turn.completed");
  const outcomeEvents = events.filter((event2) => ["approval", "decision", "summary", "tool.completed", "turn.completed"].includes(event2.kind));
  const eventManifestHash = sha256(events.map((event2) => ({
    eventId: event2.eventId,
    hash: event2.hash,
    kind: event2.kind,
    timestamp: event2.timestamp,
  ...
  const lifecycleCore = {
    observedState: completedTurns.length > 0 ? "turn-completed" : events.some((event2) => event2.kind === "session.started") ? "started" : "active",
    firstEventId: events[0]?.eventId ?? null,
    lastEventId: events.at(-1)?.eventId ?? null,
    sessionStartEvents: events.filter((event2) => event2.kind === "session.started").length,
    promptEvents: events.filter((event2) => event2.kind === "prompt.submitted").length,
    completedTurns: completedTurns.length,
    compactionEvents: events.filter((event2) => event2.kind === "compaction.started" || event2.kind === "compaction.completed").length,
    turnIds,
    kindCounts
  };
  const outcomeCore = {
    eventCount: outcomeEvents.length,
    eventIds: outcomeEvents.map((event2) => event2.eventId),
    manifestHash: sha256(outcomeEvents.map((event2) => ({ eventId: event2.eventId, hash: event2.hash, kind: event2.kind })))
  };
  const base = {
    ...
    hostAdapters: [...new Set(events.map((event2) => event2.provenance.adapter))].sort(),
    interval: {
    ...
    delivered: {
      ...
      sourceEventsSelected: events.filter((event2) => selected3.has(event2.eventId)).length,
...
async function buildSessionContextReceipts(options = {}) {
  ...
  const grouped = /* @__PURE__ */ new Map();
  for (const event2 of events) {
    if (event2.sessionId === null) continue;
    if (normalized.sessionId !== void 0 && event2.sessionId !== normalized.sessionId) continue;
    const list2 = grouped.get(event2.sessionId) ?? [];
    list2.push(event2);
    grouped.set(event2.sessionId, list2);
  }
  const receipts = [];
...
async function buildMemoryDashboard(options = {}) {
  ...
  for (const event2 of events) {
    for (const relation of event2.relations) {
      ...
      if (relation.type === "contradicts" && byId.has(relation.target)) {
        conflicts.push({ source: eventSummary(event2), target: eventSummary(byId.get(relation.target)) });
      }
    }
  }
  const decisions = events.filter((event2) => event2.kind === "decision");
  const projectRecords = buildProjectRecordViews(events, workspace.config.workspaceId);
  const tools = events.filter((event2) => event2.kind === "tool.requested" || event2.kind === "tool.completed");
  const latestStructure3 = [...events].reverse().find((event2) => event2.data?.projectStructure?.files);
  ...
  const repositoryIds = [...new Set(events.map((event2) => event2.repository?.id).filter(Boolean))].sort();
  ...
  return deepFreezeJson({
    ...
    totals: {
      ...
      currentDecisions: decisions.filter((event2) => !superseded.has(event2.eventId)).length,
      supersededDecisions: decisions.filter((event2) => superseded.has(event2.eventId)).length,
      conflicts: conflicts.length,
      tools: tools.length,
      flowSteps: projectRecords.flow.length,
      majorChanges: projectRecords.majorChanges.length,
      citedSources: new Set(events.map((event2) => event2.provenance.sourceId).filter(Boolean)).size,
    ...
    tools: tools.slice(-100).reverse().map((event2) => ({
      ...eventSummary(event2),
      sessionId: event2.sessionId,
      turnId: event2.turnId,
      toolName: typeof event2.data?.toolName === "string" ? event2.data.toolName : event2.title,
      result: event2.kind === "tool.completed" ? event2.body : ""
    })),
    ...
    conflicts,
    citations: events.filter((event2) => event2.provenance.sourceId).map(eventSummary),
...
import { lstat as lstat12, readFile as readFile7 } from "node:fs/promises";
import os3 from "node:os";
import path18 from "node:path";
init_workspace();
var ACTIVATION_SCHEMA_VERSION = "qarinah.activation.v1";
var ACTIVATION_CONSENT_VERSION = "2026-08-22";
var ACTIVATION_ENDPOINT = "https://qarinah.io/api/activation";
var MAX_STATE_BYTES = 32 * 1024;
...
async function readState(workspace) {
  ...
  let metadata;
  try {
    metadata = await lstat12(statePath);
  } catch (error) {
    if (error?.code === "ENOENT") return null;
    throw error;
  }
  ...
  const state = JSON.parse(await readFile7(statePath, "utf8"));
  if (state?.schemaVersion !== ACTIVATION_SCHEMA_VERSION || typeof state.enabled !== "boolean" || !/^[0-9a-f-]{36}$/u.test(state.installationId ?? "") || !Number.isFinite(Date.parse(state.firstSeenAt ?? "")) || typeof state.sent !== "object" || state.sent === null || Array.isArray(state.sent)) {
...
async function configureActivationTracking(options = {}) {
  const workspace = await loadWorkspace(path18.resolve(options.cwd ?? process.cwd()));
  const current = await readState(workspace);
  if (options.enabled !== true) {
    if (!current) return disabledStatus();
    const next2 = { ...current.state, enabled: false, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
    await writeState(current.statePath, next2);
...
async function activationTrackingStatus(options = {}) {
  let workspace;
  try {
    workspace = await loadWorkspace(path18.resolve(options.cwd ?? process.cwd()));
  } catch (error) {
  ...
  return Object.freeze({
    ...
    sentEvents: Object.keys(current.state.sent).filter((event2) => EVENTS.has(event2)).sort()
  });
}
async function recordActivationEvent(event2, options = {}) {
  if (!EVENTS.has(event2)) throw new TypeError(`Unsupported activation event '${event2}'.`);
  let workspace;
  try {
    workspace = await loadWorkspace(path18.resolve(options.cwd ?? process.cwd()));
  } catch (error) {
  ...
  const current = await readState(workspace);
  if (!current || current.state.enabled !== true) return disabledStatus();
  if (current.state.sent[event2]) return Object.freeze({ enabled: true, sent: false, reason: "already-sent", event: event2 });
  if (event2 === "seven_day_return" && Date.now() - Date.parse(current.state.firstSeenAt) < 7 * 24 * 60 * 60 * 1e3) {
    return Object.freeze({ enabled: true, sent: false, reason: "not-due", event: event2 });
  }
  const payload = {
    schemaVersion: ACTIVATION_SCHEMA_VERSION,
    consentVersion: current.state.consentVersion,
  ...
  try {
    response = await fetch(endpoint, {
      ...
      signal: AbortSignal.timeout(options.timeoutMs ?? 2e3)
    });
  } catch {
    return Object.freeze({ enabled: true, sent: false, reason: "endpoint-unavailable", event: event2 });
  }
  if (!response.ok) return Object.freeze({ enabled: true, sent: false, reason: `endpoint-${response.status}`, event: event2 });
  const next = {
    ...current.state,
    updatedAt: payload.occurredAt,
    sent: { ...current.state.sent, [event2]: payload.occurredAt }
  };
  await writeState(current.statePath, next);
  return Object.freeze({ enabled: true, sent: true, event: event2 });
}
...
var PACKAGE_ROOT = path19.resolve(path19.dirname(fileURLToPath3(import.meta.url)), "..");
var BIN_PATH = path19.join(PACKAGE_ROOT, "bin", "qarinah.js");
...
async function safeRead(candidate, label) {
  ...
  return readFile8(candidate, "utf8");
}
async function ensureDirectory(candidate, root, label) {
  ...
  resolveWithin(root, path19.relative(root, candidate));
}
...
function addHookEvents(settings, adapter, options) {
  ...
  for (const event2 of events) {
    const entries = Array.isArray(next.hooks[event2]) ? [...next.hooks[event2]] : [];
    ...
    if (captureIndex === -1) {
      entries.push({ ...event2.includes("Tool") || event2.startsWith("Subagent") ? { matcher: "*" } : {}, hooks: [captureHook] });
      captureIndex = entries.length - 1;
    }
    if (event2 === "Stop" && options.autoCompact === true) {
...
async function installSkill(workspace, host, skillName, files) {
  ...
  const sourceRoot = path19.join(PACKAGE_ROOT, "integrations", host, "qarinah", "skills", skillName);
  for (const relative of files) {
    const destinationDirectory = path19.dirname(resolveWithin(skillRoot, relative));
    if (destinationDirectory !== skillRoot) {
      await ensureDirectory(
        destinationDirectory,
        workspace.root,
        `.${host}/skills/${skillName}/${path19.relative(skillRoot, destinationDirectory)}`
      );
    }
    const contents = await readFile8(path19.join(sourceRoot, relative), "utf8");
    const destination = resolveWithin(skillRoot, relative);
    const existing = await safeRead(destination, `${host} Qarinah skill`);
    if (existing !== null && existing !== contents) {
      throw new QarinahError(
        "SETUP_CONFLICT",
        `${path19.relative(workspace.root, destination)} already exists with different content. Preserve it or remove it before setup.`
      );
    }
...
async function installHostSkills(workspace, host) {
  await installSkill(workspace, host, "qarinah-context", [
    "SKILL.md",
    path19.join("references", "event-contract.md")
  ]);
...
async function setupWorkspace(options = {}) {
  const target = path19.resolve(options.cwd ?? process.cwd());
  let workspace;
  let exactConfigExists = false;
  try {
    await lstat13(path19.join(target, ".qarinah", "config.json"));
  ...
  if (options.shareActivation === true) {
    ...
    const captureEvent = projectStructure.captured ? await recordActivationEvent("first_capture", { cwd: workspace.root }) : null;
  ...
  return Object.freeze({
    ...
    firstRun: Object.freeze({
      message: projectStructure.captured ? `Qarinah is ready. It mapped ${projectStructure.fileCount} project files and built a cited local memory graph.` : "Qarinah is ready. Its local memory graph is current.",
      dashboard: dashboard.output,
...
import { lstat as lstat14, mkdir as mkdir9, readFile as readFile9, realpath as realpath10, unlink } from "node:fs/promises";
import path20 from "node:path";
import { fileURLToPath as fileURLToPath4 } from "node:url";
init_workspace();
var HOST_INSTALL_MANIFEST_SCHEMA_VERSION = "qarinah.host-install-manifest.v1";
var PACKAGE_ROOT2 = path20.resolve(path20.dirname(fileURLToPath4(import.meta.url)), "..");
var BIN_PATH2 = path20.join(PACKAGE_ROOT2, "bin", "qarinah.js");
...
async function inspectFile(root, relativePath) {
  ...
  let metadata;
  try {
    metadata = await lstat14(candidate);
  } catch (error) {
...
async function previewHostInstall(options = {}) {
  ...
  const binary = await readFile9(BIN_PATH2);
  const files = [];
...
async function installHostIntegration(options = {}) {
  ...
  await mkdir9(path20.dirname(output), { recursive: true, mode: 448 });
...
function removeHooks(value, binaryPath) {
  ...
  for (const [event2, entries] of Object.entries(next.hooks)) {
    ...
    if (retained.length > 0) next.hooks[event2] = retained;
    else delete next.hooks[event2];
  }
...
async function transformedContents(root, entry, manifest) {
  ...
  const contents = await readFile9(candidate, "utf8");
...
async function uninstallHostIntegration(options = {}) {
  ...
  let manifest;
  try {
    manifest = JSON.parse(await readFile9(manifestPath, "utf8"));
  } catch {
...
import { lstat as lstat15, readFile as readFile10, realpath as realpath11 } from "node:fs/promises";
...
async function inspectFile2(workspace, file) {
  ...
  let metadata;
  try {
    metadata = await lstat15(absolute);
  } catch (error) {
  ...
  const relative = path21.relative(workspace.root, resolved);
  ...
  const observedHash = hashBytes3(await readFile10(resolved));
...
async function dependencyFreshness(events, resolver) {
  ...
  for (const event2 of events) {
    for (const dependency of event2.freshness?.dependencies ?? []) {
      latest.set(dependency.name, { ...dependency, eventId: event2.eventId, repository: event2.repository ?? null });
    }
  }
  const results = [];
...
async function inspectMemoryFreshness(options = {}) {
  ...
  const staleEventIds = events.filter((event2) => (event2.freshness?.files ?? []).some((file) => stalePaths.has(file.path)) || (event2.freshness?.dependencies ?? []).some((entry) => staleDependencies.has(entry.name))).map((event2) => event2.eventId).sort();
  return deepFreezeJson({
    schemaVersion: "qarinah.memory-freshness.v1",
    workspaceId: workspace.config.workspaceId,
...
function timeline(dashboard) {
  return dashboard.activity.map((event2) => ({
    eventId: event2.eventId,
    timestamp: event2.timestamp,
    kind: event2.kind,
    title: event2.title,
    hash: event2.hash,
    repositoryId: event2.repositoryId,
    sourceId: event2.sourceId,
    category: event2.kind === "decision" ? "decision" : event2.kind.startsWith("tool.") ? "tool" : event2.kind === "turn.completed" || event2.kind === "summary" ? "outcome" : "activity"
  }));
}
...
// src/demo.js
init_store();
import { mkdtemp, mkdir as mkdir10, readFile as readFile11, unlink as unlink2, writeFile as writeFile2 } from "node:fs/promises";
...
async function createRoot(output) {
  if (!output) return mkdtemp(path23.join(os4.tmpdir(), "qarinah-demo-"));
  const root = path23.resolve(output);
  await mkdir10(root, { recursive: false });
  return root;
}
...
async function createDemoWorkspace(options = {}) {
  ...
  await mkdir10(path23.join(root, "src"));
  await writeFile2(path23.join(root, "package.json"), `${JSON.stringify({ name: "qarinah-handoff-demo", type: "module" }, null, 2)}
`, "utf8");
  await writeFile2(path23.join(root, "src", "retry-policy.js"), [
  ...
  await writeFile2(path23.join(root, "README.md"), "# Checkout service\n\nThe retry decision is preserved by Qarinah for the next coding-agent session.\n", "utf8");
  const transientTranscript = path23.join(root, "session-a-transcript.txt");
  await writeFile2(transientTranscript, [
  ...
  await unlink2(transientTranscript);
  ...
  return Object.freeze({
    ...
    readme: await readFile11(path23.join(root, "README.md"), "utf8")
  });
}
...
async function run(argv) {
  ...
  if (!["setup", "demo", "activation", "mcp", "hook"].includes(command)) {
    await recordActivationEvent("seven_day_return", { cwd: process2.cwd() });
  }
  if (command === "demo") {
    ...
    process2.stdout.write(`${JSON.stringify(await createDemoWorkspace({ output: parsed.values.get("--output") }), null, 2)}
`);
    return;
  }
  if (command === "activation") {
    ...
    const result = args[0] === "status" ? await activationTrackingStatus({ cwd: process2.cwd() }) : await configureActivationTracking({ cwd: process2.cwd(), enabled: args[0] === "enable" });
...
`);
  ...
  if (command === "setup") {
    const flags = /* @__PURE__ */ new Set(["--codex", "--claude", "--cursor", "--kimi", "--antigravity", "--freebuff", "--allow-query", "--auto-compact", "--share-activation"]);
    ...
    const result = await setupWorkspace({
      ...
      backupSources: parsed.values.has("--backup-source") ? [path24.resolve(parsed.values.get("--backup-source"))] : void 0,
      backupDestination: parsed.values.has("--backup-destination") ? path24.resolve(parsed.values.get("--backup-destination")) : void 0,
...
`);
  ...
  if (command === "record") {
    ...
    const event2 = await appendEvent(input);
    process2.stdout.write(`${JSON.stringify(event2, null, 2)}
`);
    return;
  }
  if (command === "hook") {
...
`);
  ...
  if (command === "scan") {
    ...
    if (result.captured) await recordActivationEvent("first_capture", { cwd: process2.cwd() });
    process2.stdout.write(`${JSON.stringify(result, null, 2)}
`);
    return;
  }
...
`);
  ...
  if (command === "backup") {
    ...
    const result = await backupAgentArchives(
      parsed.positionals.map((source) => path24.resolve(source)),
      path24.resolve(destination),
      {
        cwd: process2.cwd(),
        maxBytes: integer6("--max-bytes"),
        maxFiles: integer6("--max-files")
...
`);
  ...
  if (command === "query" || command === "context") {
    ...
    await recordActivationEvent("first_retrieval", { cwd: process2.cwd() });
...
`);
      ...
      await recordActivationEvent("first_cross_session_handoff", { cwd: process2.cwd() });
