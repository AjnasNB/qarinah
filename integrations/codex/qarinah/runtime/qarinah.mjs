#!/usr/bin/env node
// GENERATED by scripts/build-plugins.mjs for Qarinah 0.1.0-alpha.1. DO NOT EDIT.
...
function sanitizeJsonValue(value, options = {}) {
  ...
  let nodes = 0;
  function visit(candidate, path6, depth) {
    nodes += 1;
    if (nodes > limits.maximumNodes) throw new TypeError(`JSON value exceeds ${limits.maximumNodes} nodes.`);
    if (depth > limits.maximumDepth) throw new TypeError(`JSON value exceeds depth ${limits.maximumDepth} at ${path6}.`);
...
[QARINAH_TRUNCATED:${candidate.length - limits.maximumStringLength}]`;
    ...
    if (typeof candidate === "number") {
      if (!Number.isFinite(candidate)) throw new TypeError(`${path6} must contain only finite numbers.`);
      return Object.is(candidate, -0) ? 0 : candidate;
    }
    if (Array.isArray(candidate)) {
      if (candidate.length > limits.maximumArrayLength) {
        throw new TypeError(`${path6} exceeds ${limits.maximumArrayLength} array items.`);
      }
      ...
      for (const key of Reflect.ownKeys(descriptors2)) {
        if (typeof key !== "string" || !allowed.has(key)) throw new TypeError(`${path6} contains an unsupported array property.`);
      }
      return Array.from({ length: candidate.length }, (_, index) => {
        const descriptor = descriptors2[String(index)];
        if (!descriptor?.enumerable || !Object.hasOwn(descriptor, "value")) {
          throw new TypeError(`${path6}[${index}] must be an enumerable data property.`);
        }
        return visit(descriptor.value, `${path6}[${index}]`, depth + 1);
      });
    }
    if (!candidate || typeof candidate !== "object") throw new TypeError(`${path6} contains an unsupported value.`);
    const prototype = Object.getPrototypeOf(candidate);
    if (prototype !== Object.prototype && prototype !== null) {
      throw new TypeError(`${path6} must contain only plain records.`);
    }
    const descriptors = Object.getOwnPropertyDescriptors(candidate);
    const keys = Reflect.ownKeys(descriptors);
    if (keys.length > limits.maximumObjectKeys) throw new TypeError(`${path6} exceeds ${limits.maximumObjectKeys} fields.`);
    const result = /* @__PURE__ */ Object.create(null);
    for (const key of keys) {
      if (typeof key !== "string") throw new TypeError(`${path6} cannot contain symbol keys.`);
      const descriptor = descriptors[key];
      if (!descriptor.enumerable || !Object.hasOwn(descriptor, "value") || descriptor.value === void 0) {
        throw new TypeError(`${path6}.${key} must be an enumerable data property with a defined value.`);
      }
      result[key] = visit(descriptor.value, `${path6}.${key}`, depth + 1);
    }
    return result;
  }
...
function snapshotJsonBoundary(value, options = {}) {
  ...
  function visit(candidate, path6, depth) {
    ...
    if (depth > limits.maximumDepth) throw new TypeError(`${limits.label} exceeds depth ${limits.maximumDepth} at ${path6}.`);
    if (candidate === null || typeof candidate === "boolean") return candidate;
    if (typeof candidate === "string") {
      if (candidate.length > limits.maximumStringLength) {
        throw new TypeError(`${path6} exceeds ${limits.maximumStringLength} characters.`);
      }
      return candidate;
    }
    if (typeof candidate === "number") {
      if (!Number.isFinite(candidate) || Object.is(candidate, -0)) {
        throw new TypeError(`${path6} must be a finite JSON number other than negative zero.`);
      }
      return candidate;
    }
    if (!candidate || typeof candidate !== "object") throw new TypeError(`${path6} contains a non-JSON value.`);
    if (seen.has(candidate)) throw new TypeError(`${path6} contains a cyclic or repeated object reference.`);
    seen.add(candidate);
    if (Array.isArray(candidate)) {
      if (candidate.length > limits.maximumArrayLength) {
        throw new TypeError(`${path6} exceeds ${limits.maximumArrayLength} array entries.`);
      }
      const descriptors2 = Object.getOwnPropertyDescriptors(candidate);
      const expected = /* @__PURE__ */ new Set(["length", ...Array.from({ length: candidate.length }, (_, index) => String(index))]);
      for (const key of Reflect.ownKeys(descriptors2)) {
        if (typeof key !== "string" || !expected.has(key)) throw new TypeError(`${path6} contains an unsupported array property.`);
      }
      return Array.from({ length: candidate.length }, (_, index) => {
        const descriptor = descriptors2[String(index)];
        if (!descriptor?.enumerable || !Object.hasOwn(descriptor, "value")) {
    ...
    const prototype = Object.getPrototypeOf(candidate);
    if (prototype !== Object.prototype && prototype !== null) throw new TypeError(`${path6} must be a plain record.`);
    const descriptors = Object.getOwnPropertyDescriptors(candidate);
    const keys = Reflect.ownKeys(descriptors);
    if (keys.length > limits.maximumObjectKeys) throw new TypeError(`${path6} exceeds ${limits.maximumObjectKeys} fields.`);
    const result = /* @__PURE__ */ Object.create(null);
...
var init_contracts = __esm({
  "src/contracts.js"() {
    ...
    CONTEXT_PACK_SCHEMA_VERSION = "qarinah.context-pack.v1";
...
// src/version.js
var QARINAH_VERSION = "0.1.0-alpha.1";
...
import { lstat as lstat4, mkdir as mkdir4, readFile, readdir as readdir2, realpath as realpath3, rename as rename4, rm as rm4 } from "node:fs/promises";
...
function resolveOutputDirectory(workspace, output) {
  ...
  const relative = path3.relative(workspace.root, outputDirectory);
  if (relative === "") {
    throw new QarinahError("OKF_OUTPUT_PROTECTED", "The workspace root cannot be replaced by an OKF export.");
  }
  const segments = relative.split(path3.sep).filter(Boolean);
...
async function assertOwnedOutput(outputDirectory, workspace) {
  ...
  const actual = await realpath3(outputDirectory);
  ...
  const entries = (await readdir2(outputDirectory)).sort(compareText);
  ...
  const eventDirectory = path3.join(outputDirectory, "events");
  const eventMetadata = await lstat4(eventDirectory);
  ...
  const eventFiles = (await readdir2(eventDirectory)).sort(compareText);
  ...
  await assertRegularFile(path3.join(outputDirectory, EXPORT_MARKER), "OKF ownership marker", 64 * 1024);
  await assertRegularFile(path3.join(outputDirectory, "index.md"), "OKF root index", 64 * 1024 * 1024);
  await assertRegularFile(path3.join(outputDirectory, "log.md"), "OKF root log", 64 * 1024 * 1024);
  for (const name of eventFiles) {
    await assertRegularFile(path3.join(eventDirectory, name), `OKF event concept '${name}'`, 2 * 1024 * 1024);
  }
  const markerText = await readFile(path3.join(outputDirectory, EXPORT_MARKER), "utf8");
  let marker;
  try {
...
`) {
  ...
  const existingFiles = /* @__PURE__ */ new Map([
    ["index.md", await readFile(path3.join(outputDirectory, "index.md"), "utf8")],
    ["log.md", await readFile(path3.join(outputDirectory, "log.md"), "utf8")]
  ]);
  let totalBytes = Buffer.byteLength(existingFiles.get("index.md")) + Buffer.byteLength(existingFiles.get("log.md"));
  for (const name of eventFiles) {
    const contents = await readFile(path3.join(eventDirectory, name), "utf8");
...
async function replaceOutput(stage, outputDirectory, workspace) {
  ...
  const parent = path3.dirname(outputDirectory);
  const backup = path3.join(parent, `.${path3.basename(outputDirectory)}.qarinah-okf-backup-${process.pid}-${randomBytes3(8).toString("hex")}`);
  resolveWithin(workspace.root, path3.relative(workspace.root, backup));
...
async function exportOkf(options = {}) {
  ...
  const stage = path3.join(parent, `.${path3.basename(outputDirectory)}.qarinah-okf-stage-${process.pid}-${randomBytes3(8).toString("hex")}`);
  resolveWithin(workspace.root, path3.relative(workspace.root, stage));
...
function hookPayload2(input, ignoredFieldCount, workspace) {
  ...
  const requestedCwd = path5.resolve(input.cwd);
  const relativeCwd = path5.relative(workspace.root, requestedCwd);
  const workspaceRelativeCwd = relativeCwd === "" || !relativeCwd.startsWith("..") && !path5.isAbsolute(relativeCwd) ? relativeCwd || "." : "[outside-workspace]";
  const metadata = {
    host: "claude-code",
    hookEvent: eventName,
