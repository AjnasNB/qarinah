...
</style></head><body><header><div class="eyebrow">Visible developer memory</div><h1>${escapeHtml(data.workspace.name || "Qarinah")}</h1><input type="search" data-search placeholder="Search nodes, decisions, tools, outcomes..."><div class="metrics"><div class="metric"><strong>${data.health.graphNodes}</strong><span>graph nodes</span></div><div class="metric"><strong>${data.timeline.length}</strong><span>events</span></div><div class="metric"><strong>${data.sessions.receiptCount}</strong><span>sessions</span></div><div class="metric"><strong>${data.worktreeComparison.worktreeCount}</strong><span>worktrees</span></div></div><nav><button data-tab="graph" aria-selected="true">Graph</button><button data-tab="timeline">Timeline</button><button data-tab="sessions">Sessions</button><button data-tab="worktrees">Worktrees</button></nav></header><main><section data-panel="graph" data-active="true"><svg class="graph" viewBox="0 0 400 320" data-graph role="img" aria-label="Interactive Qarinah project-memory graph"></svg><div class="cards" data-graph-list></div></section><section data-panel="timeline"><div class="cards" data-timeline></div></section><section data-panel="sessions"><div class="cards" data-sessions></div></section><section data-panel="worktrees"><div class="cards" data-worktrees></div></section></main><script nonce="${token}">
