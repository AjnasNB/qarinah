#!/usr/bin/env node
// GENERATED by scripts/build-plugins.mjs for Qarinah 0.3.0. DO NOT EDIT.
...
function sanitizeJsonValue(value, options = {}) {
  ...
  let nodes = 0;
  function visit(candidate, path20, depth) {
    nodes += 1;
    if (nodes > limits.maximumNodes) throw new TypeError(`JSON value exceeds ${limits.maximumNodes} nodes.`);
    if (depth > limits.maximumDepth) throw new TypeError(`JSON value exceeds depth ${limits.maximumDepth} at ${path20}.`);
...
[QARINAH_TRUNCATED:${candidate.length - limits.maximumStringLength}]`;
    ...
    if (typeof candidate === "number") {
      if (!Number.isFinite(candidate)) throw new TypeError(`${path20} must contain only finite numbers.`);
      return Object.is(candidate, -0) ? 0 : candidate;
    }
    if (Array.isArray(candidate)) {
      if (candidate.length > limits.maximumArrayLength) {
        throw new TypeError(`${path20} exceeds ${limits.maximumArrayLength} array items.`);
      }
      ...
      for (const key of Reflect.ownKeys(descriptors2)) {
        if (typeof key !== "string" || !allowed.has(key)) throw new TypeError(`${path20} contains an unsupported array property.`);
      }
      return Array.from({ length: candidate.length }, (_, index) => {
        const descriptor = descriptors2[String(index)];
        if (!descriptor?.enumerable || !Object.hasOwn(descriptor, "value")) {
          throw new TypeError(`${path20}[${index}] must be an enumerable data property.`);
        }
        return visit(descriptor.value, `${path20}[${index}]`, depth + 1);
      });
    }
    if (!candidate || typeof candidate !== "object") throw new TypeError(`${path20} contains an unsupported value.`);
    const prototype = Object.getPrototypeOf(candidate);
    if (prototype !== Object.prototype && prototype !== null) {
      throw new TypeError(`${path20} must contain only plain records.`);
    }
    const descriptors = Object.getOwnPropertyDescriptors(candidate);
    const keys = Reflect.ownKeys(descriptors);
    if (keys.length > limits.maximumObjectKeys) throw new TypeError(`${path20} exceeds ${limits.maximumObjectKeys} fields.`);
    const result = /* @__PURE__ */ Object.create(null);
    for (const key of keys) {
      if (typeof key !== "string") throw new TypeError(`${path20} cannot contain symbol keys.`);
      const descriptor = descriptors[key];
      if (!descriptor.enumerable || !Object.hasOwn(descriptor, "value") || descriptor.value === void 0) {
        throw new TypeError(`${path20}.${key} must be an enumerable data property with a defined value.`);
      }
      result[key] = visit(descriptor.value, `${path20}.${key}`, depth + 1);
    }
    return result;
  }
...
function snapshotJsonBoundary(value, options = {}) {
  ...
  function visit(candidate, path20, depth) {
    ...
    if (depth > limits.maximumDepth) throw new TypeError(`${limits.label} exceeds depth ${limits.maximumDepth} at ${path20}.`);
    if (candidate === null || typeof candidate === "boolean") return candidate;
    if (typeof candidate === "string") {
      if (candidate.length > limits.maximumStringLength) {
        throw new TypeError(`${path20} exceeds ${limits.maximumStringLength} characters.`);
      }
      return candidate;
    }
    if (typeof candidate === "number") {
      if (!Number.isFinite(candidate) || Object.is(candidate, -0)) {
        throw new TypeError(`${path20} must be a finite JSON number other than negative zero.`);
      }
      return candidate;
    }
    if (!candidate || typeof candidate !== "object") throw new TypeError(`${path20} contains a non-JSON value.`);
    if (seen.has(candidate)) throw new TypeError(`${path20} contains a cyclic or repeated object reference.`);
    seen.add(candidate);
    if (Array.isArray(candidate)) {
      if (candidate.length > limits.maximumArrayLength) {
        throw new TypeError(`${path20} exceeds ${limits.maximumArrayLength} array entries.`);
      }
      const descriptors2 = Object.getOwnPropertyDescriptors(candidate);
      const expected = /* @__PURE__ */ new Set(["length", ...Array.from({ length: candidate.length }, (_, index) => String(index))]);
      for (const key of Reflect.ownKeys(descriptors2)) {
        if (typeof key !== "string" || !expected.has(key)) throw new TypeError(`${path20} contains an unsupported array property.`);
      }
      return Array.from({ length: candidate.length }, (_, index) => {
        const descriptor = descriptors2[String(index)];
        if (!descriptor?.enumerable || !Object.hasOwn(descriptor, "value")) {
    ...
    const prototype = Object.getPrototypeOf(candidate);
    if (prototype !== Object.prototype && prototype !== null) throw new TypeError(`${path20} must be a plain record.`);
    const descriptors = Object.getOwnPropertyDescriptors(candidate);
    const keys = Reflect.ownKeys(descriptors);
    if (keys.length > limits.maximumObjectKeys) throw new TypeError(`${path20} exceeds ${limits.maximumObjectKeys} fields.`);
    const result = /* @__PURE__ */ Object.create(null);
...
function trustPath(root) {
  ...
  return path2.join(stateRoot(), "trusted-workspaces", `${digest}.json`);
}
...
async function setupWorkspace(options = {}) {
  ...
  workspace = exactConfigExists ? await loadWorkspace(target) : await initializeWorkspace(target, { capture: options.capture ?? "metadata" });
...
async function inspectFile(workspace, file) {
  ...
  let metadata;
  try {
    metadata = await lstat11(absolute);
  } catch (error) {
  ...
  const resolved = await realpath9(absolute);
  const relative = path17.relative(workspace.root, resolved);
...
`);
  ...
  if (command === "setup") {
    const flags = /* @__PURE__ */ new Set(["--codex", "--claude", "--cursor", "--kimi", "--antigravity", "--allow-query", "--auto-compact"]);
    ...
    const result = await setupWorkspace({
      ...
      backupSources: parsed.values.has("--backup-source") ? [path19.resolve(parsed.values.get("--backup-source"))] : void 0,
      backupDestination: parsed.values.has("--backup-destination") ? path19.resolve(parsed.values.get("--backup-destination")) : void 0,
...
`);
  ...
  if (command === "backup") {
    ...
    const result = await backupAgentArchives(
      parsed.positionals.map((source) => path19.resolve(source)),
      path19.resolve(destination),
      {
        cwd: process2.cwd(),
        maxBytes: integer3("--max-bytes"),
        maxFiles: integer3("--max-files")
