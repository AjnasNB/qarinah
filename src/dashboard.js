...
.graph-toolbar{display:grid;grid-template-columns:minmax(220px,1fr) 190px auto auto;gap:10px;align-items:end;margin-bottom:14px}.graph-toolbar label{display:grid;gap:6px;color:var(--muted);font-size:12px}.graph-toolbar input,.graph-toolbar select{width:100%;min-height:44px;border:1px solid var(--line);border-radius:8px;background:#0b1118;color:var(--text);padding:9px 12px;font:inherit}.graph-toolbar input:focus-visible,.graph-toolbar select:focus-visible{outline:2px solid var(--mint);outline-offset:2px}.graph-reset{min-height:44px;padding:9px 14px;border:1px solid var(--line);border-radius:8px;background:#17212d;color:var(--text);font:700 12px/1 ui-monospace,monospace;cursor:pointer}.graph-reset:hover,.graph-reset:focus-visible{border-color:var(--mint);color:var(--mint);outline:none}.graph-summary{color:var(--muted);font:12px/1.35 ui-monospace,monospace}.graph-shell{display:grid;grid-template-columns:minmax(0,1.8fr) minmax(240px,.7fr);gap:14px}.graph-canvas{display:block;width:100%;height:auto;min-height:420px;border:1px solid var(--line);background:radial-gradient(circle at center,rgb(53 224 170 / 7%),transparent 55%),#0b1118;touch-action:none}.graph-canvas line{stroke:#31404d;stroke-width:1;vector-effect:non-scaling-stroke}.graph-node{cursor:grab}.graph-node[data-dragging="true"]{cursor:grabbing}.graph-node circle{stroke:#090d12;stroke-width:2;vector-effect:non-scaling-stroke}.graph-node .graph-halo{fill:none;stroke:var(--mint);stroke-width:2;opacity:0;pointer-events:none}.graph-node:focus-visible circle,.graph-node[data-selected="true"] .graph-dot{stroke:var(--text);stroke-width:3}.graph-node[data-selected="true"] .graph-halo{opacity:.55;animation:qarinah-graph-pulse 1.8s ease-out infinite}.graph-node[data-conflict="true"] .graph-dot{stroke:var(--warn);stroke-width:3}.graph-details{border:1px solid var(--line);padding:16px;min-width:0}.graph-details h3{margin:0 0 8px;font-size:18px;overflow-wrap:anywhere}.graph-details dl{display:grid;grid-template-columns:auto minmax(0,1fr);gap:7px 11px;margin:14px 0}.graph-details dt{color:var(--muted)}.graph-details dd{margin:0;overflow-wrap:anywhere}.graph-results{display:grid;gap:6px;max-height:235px;overflow:auto;list-style:none;margin:8px 0 0;padding:0 3px 0 0}.graph-results li{display:block;padding:0;border:0}.graph-results button{display:grid;gap:2px;width:100%;padding:8px 10px;border:1px solid var(--line);border-radius:7px;background:#0b1118;color:var(--text);text-align:left;cursor:pointer}.graph-results button:hover,.graph-results button:focus-visible,.graph-results button[data-selected="true"]{border-color:var(--mint);outline:none}.graph-results small{color:var(--muted)}.graph-legend{display:flex;flex-wrap:wrap;gap:10px 16px;margin:10px 0 0;color:var(--muted);font-size:12px}.graph-legend span{display:inline-flex;align-items:center;gap:6px}.graph-legend i{width:10px;height:10px;border-radius:50%;background:var(--legend)}
...
@media(max-width:900px){.graph-toolbar{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}.graph-summary{grid-column:1/-1}.graph-shell{grid-template-columns:1fr}.graph-canvas{min-height:330px}.graph-details{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:14px}.graph-details h3,.graph-details>p{grid-column:1/-1}}
...
const qarinahRenderGraph=()=>{
  ...
  for(const edge of qarinahGraph.edges){const source=positions.get(edge.source),target=positions.get(edge.target);if(!source||!target)continue;const line=qarinahSvgElement("line");line.dataset.source=edge.source;line.dataset.target=edge.target;line.setAttribute("x1",source.x);line.setAttribute("y1",source.y);line.setAttribute("x2",target.x);line.setAttribute("y2",target.y);line.setAttribute("opacity",String(Math.max(.18,Math.min(.82,edge.weight))));const title=qarinahSvgElement("title");title.textContent=edge.type;line.append(title);qarinahGraphEdges.append(line)}
  qarinahGraphNodes.textContent="";
  for(const node of visible){const point=positions.get(node.id);const group=qarinahSvgElement("g");group.classList.add("graph-node");group.dataset.nodeId=node.id;group.dataset.conflict=String(node.conflicted);group.dataset.selected=String(node.id===qarinahSelectedNode);group.dataset.dragging="false";group.setAttribute("transform","translate("+point.x+" "+point.y+")");group.setAttribute("tabindex","0");group.setAttribute("role","button");group.setAttribute("aria-label",node.type+": "+node.label+". Drag to move; press Enter for details.");const halo=qarinahSvgElement("circle");halo.classList.add("graph-halo");halo.setAttribute("r","10");const circle=qarinahSvgElement("circle");circle.classList.add("graph-dot");circle.setAttribute("r",String(6+node.importance*8));circle.setAttribute("fill",qarinahGraphColors[node.type]);const title=qarinahSvgElement("title");title.textContent=node.label+" | "+node.type;group.append(halo,circle,title);const activate=()=>qarinahShowNode(node);group.addEventListener("pointerdown",(event)=>{if(event.button!==0)return;group.dataset.dragging="true";qarinahActiveDrag={node,group,positions,pointerId:event.pointerId};qarinahGraphSvg.setPointerCapture(event.pointerId);activate();event.preventDefault()});group.addEventListener("click",activate);group.addEventListener("keydown",(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();activate()}});qarinahGraphNodes.append(group)}
